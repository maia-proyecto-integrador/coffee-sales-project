...
# Asegurar que datetime sea sin zona horaria
df["datetime"] = dd.to_datetime(df["datetime"], errors="coerce", utc=True).dt.tz_convert(None)
...
# Renombrar coffee_name si tiene espacios raros
...
def update_kpis(drink, period, dow, hour_range, model):
    dff = df[
...
def patrones_compra(drink, period, dow, hour_range, model):
    dff = df[
        (df["datetime"] >= pd.to_datetime(period[0])) &
        (df["datetime"] <= pd.to_datetime(period[1]))
    ]
...
def forecast_panel(drink, period, dow, hour_range, model, horizon=15, return_data=False):
    # 📂 Filtros
    dff = df[
        (df["datetime"] >= pd.to_datetime(period[0])) & 
        (df["datetime"] <= pd.to_datetime(period[1]))
...
# ===========================
def validation_panel(drink, period, dow, hour_range, model, horizon=15):
    # 📂 Filtros
    dff = df[
        (df["datetime"] >= pd.to_datetime(period[0])) & 
        (df["datetime"] <= pd.to_datetime(period[1]))
...
forecast_view, forecast_data = pn.bind(
    ...
    hour_range=hour_filter, model=model_filter
)()
