...
def run_sarimax(df: pd.DataFrame, register_final: bool = True):
    """
    Corre SARIMAX con rolling origins para evaluación y, opcionalmente,
    entrena un modelo FINAL (único) sobre TODA la historia agregada (sumatoria del TARGET por día)
    para registrarlo como 'best_model' compatible con dashboard.py.

    Returns
    -------
    sarimax_results : DataFrame de predicciones por split (para métricas)
    by_h_sarimax    : DataFrame con métricas por horizonte
    overall_sarimax : dict/serie con métricas globales
    """
    sarimax_all = []
    ...
    for train_end, test_start, test_end in splits:
        ...
        print(
            f"[SARIMAX] Split {train_end.date()} | train_days={n_days} | test_days={test['date'].nunique()}"
        )
        ...
        for prod, g in train.groupby("product"):
            ...
            ex_te = None
            if exog_cols and use_exog:
                g_full = df[df["product"] == prod].set_index("date")
                ex_te = (
                    g_full[exog_cols].asfreq("D").ffill().bfill().reindex(future_idx)
                )
            ...
            try:
                ...
                if len(conf_cols) >= 2:
                    lower_col = next(
                        (c for c in conf_cols if "lower" in c.lower()), conf_cols[0]
                    )
                    upper_col = next(
                        (c for c in conf_cols if "upper" in c.lower()), conf_cols[1]
                    )
            ...
            out = pd.DataFrame(
                {"date": future_idx, "product": prod, "yhat": yhat.values}
            )
    ...
    sarimax_results = (
        pd.concat(sarimax_all, ignore_index=True)
        if sarimax_all
        else pd.DataFrame(
            columns=[
                "date",
    ...
    if sarimax_results.empty:
        # No registramos nada si no hubo resultados
        return sarimax_results, pd.DataFrame(), {}

    by_h_sarimax, overall_sarimax = summarize_metrics(
        sarimax_results.rename(columns={TARGET: "y"})
    )
    ...
    if register_final:
        ...
        y_full = agg.set_index("date")[TARGET].astype(float).asfreq("D").ffill().bfill()

        # Exógenas agregadas (promedio por día si existen)
        ex_full = None
        if exog_cols:
            ex_agg = df.groupby("date", as_index=False)[exog_cols].mean()
            ex_full = (
                ex_agg.set_index("date")
                .asfreq("D")
                .ffill()
                .bfill()
                .reindex(y_full.index)
            )

        # Elige la primera especificación que funcione (como en backtesting)
        final_res = None
        used_exog = False
        for order, seasonal_order in orders_to_try:
            try:
                ...
                final_res = m.fit(disp=False)
                used_exog = ex_full is not None
                break
            except Exception:
                try:
                    m = sm.tsa.statespace.SARIMAX(
        ...
        if final_res is not None:
            metric_name = "sMAPE"
            best_score = (
                float(overall_sarimax.get(metric_name, 0.0))
                if isinstance(overall_sarimax, dict)
                else 0.0
            )
            ...
            print("[SARIMAX] best_model actualizado:", best_entry)
        else:
            print("[SARIMAX] No se pudo refitear un modelo final para registro.")
