...
def run_sarimax(df: pd.DataFrame, register_final: bool = True):
    ...
    exog_cols = [c for c in [
        "is_holiday", "is_holiday_prev", "is_holiday_next",
        "wx_temperature_2m", "wx_precipitation", "wx_cloudcover"
    ] if c in df.columns]
    ...
    sarimax_results = (
        pd.concat(sarimax_all, ignore_index=True) if sarimax_all
    ...
    if register_final:
        ...
        if exog_cols:
            ...
            ex_full = ex_agg.set_index("date").asfreq("D").ffill().bfill().reindex(y_full.index)
        ...
        for order, seasonal_order in orders_to_try:
            try:
                m = sm.tsa.statespace.SARIMAX(
                    y_full, order=order, seasonal_order=seasonal_order,
                    exog=ex_full, enforce_stationarity=False, enforce_invertibility=False
                )
